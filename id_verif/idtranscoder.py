...
#iš pateikto personal_code kiekvieno narį prilyginame variable nuo a iki k, taippgi kiekvieną kintamąmajį padarome integeriu.
#ir su global kiekvieną kintamajį a-k padaro global kintamuoju
...
#list2 ir list3 yra daugikliai, kuriuos reikia sudauginti is kintamuju,
#gautas reiksmes susmuojam ir suskaiciuojam liekana padalinus is 11
...
def code_validator():
    if (a >= 1 or a <= 6 or a == 9) and (d == 0.1) and (sum([d + e]) <= 12) and (sum([f + g]) <= 31) and (control_number_generator() == k):
